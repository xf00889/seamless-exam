...
class ExamExportWordTests(TestCase):
    ...
    def test_exam_export_word_contains_header_footer_and_questions(self):
        ...
        self.assertIn('Generated by: valuateai.onrender.com', footer_xml)
        self.assertIn('Literature Quiz', body_text)
...
class MpsExportWordTests(TestCase):
    ...
    def test_mps_export_word_contains_header_and_footer(self):
        ...
        self.assertNotIn('Quarter MPS Overview', _document_text(document))
        self.assertNotIn('2nd Quarter', _document_text(document))
        ...
        self.assertEqual(document.tables[2].cell(0, 0).text, '#')
...
class MpsExportExcelTests(TestCase):
    ...
    def test_mps_export_excel_contains_quarter_summary_sheet(self):
        ...
        self.assertNotIn('Quarter Overview', workbook.sheetnames)
        ...
        self.assertNotIn('2nd Quarter', summary_values)
    ...
    def test_mps_export_excel_strips_illegal_cell_characters(self):
        ...
        values = {
            cell.value
            for row in workbook['Quarter Summary'].iter_rows()
            for cell in row
            if cell.value is not None
        }
        self.assertIn('BiologyCheck', values)

    def test_mps_export_excel_strips_cr_lf_in_title(self):
